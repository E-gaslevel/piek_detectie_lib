...
uint16_t expected_filtered_peaks[MAX_PEAKS];
...
int main() {
    ...
    while (fgets(f_line, sizeof(f_line), fptr) != NULL) {
        if (file_index < MAX_PEAKS) {
            expected_filtered_peaks[file_index++] = (uint16_t)atoi(f_line);
        }
    }
    fclose(fptr);

    for (size_t i = 0; i < 9; i++) {
        assert(peaks[i] == expected_filtered_peaks[i]);  // Verify filtered peaks
    }
